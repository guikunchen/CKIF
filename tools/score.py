...
def main(opt):
    # OUT_FP = open('./configs/eval_results.txt', 'w')
    # OUT_FP = open(os.path.join(os.path.dirname(opt.predictions), 'eval_results.txt'), 'w')
    OUT_FP = open(opt.predictions[:-len('-results.txt')] + '_eval' + opt.predictions[-len('-results.txt'):], 'w')
    OUT_FP.write('Reading predictions from file ... \n')
    with open(opt.predictions, 'r') as f:
        ...
        OUT_FP.write('Canonicalizing predictions using Process Number  {}\n'.format(opt.process_number))
    ...
    OUT_FP.write('data size {} \n'.format(data_size))
    OUT_FP.write('Reading targets from file ... \n')
    with open(opt.targets, 'r') as f:
        lines = f.readlines()
        # lines = [''.join(line.strip().split(' ')) for line in f.readlines()]
        print("Origin File Length", len(lines))
        OUT_FP.write('Origin File Length {} \n'.format(len(lines)))
    ...
    OUT_FP.write('Origin Target Lentgh,  {} \n'.format(len(ground_truth)))
    OUT_FP.write('Cutted Length, {} \n'.format(data_size))
    ...
    for i in range(opt.n_best):
        ...
        # if i in range(10):
            ...
            OUT_FP.write("Top-{} Acc:{:.3f}%, MaxFrag {:.3f}%,  Invalid SMILES:{:.3f}% Sorted Invalid SMILES:{:.3f}% \n".format(i+1,accuracy[i] / data_size * 100,max_frag_accuracy[i] / data_size * 100, invalid_rates[i] / data_size / opt.augmentation * 100,sorted_invalid_rates[i] / data_size / opt.augmentation * 100))

    for i in range(opt.n_best):
        if i in [0,2,4,9,19,49]:
            OUT_FP.write("{:.1f},".format(accuracy[i] / data_size * 100))
    OUT_FP.write('\n')
    for i in range(opt.n_best):
        if i in [0,2,4,9,19,49]:
            OUT_FP.write("{:.1f},".format(max_frag_accuracy[i] / data_size * 100))
    OUT_FP.write('\n')
    for i in range(opt.n_best):
        if i in [0,2,4,9,19,49]:
            OUT_FP.write("{:.1f},".format(accuracy[i] / data_size * 100))
    ...
    OUT_FP.write("Unique Rates:{:.3f}% \n".format(unique_rates / data_size / opt.beam_size * 100))
            ...
            if differ_now < 11 and differ_now != item[0]:
                for j in range(opt.n_best):
                    if (j+1) in print_topk:
                        ...
                        OUT_FP.write("Top-{} Atom differ size {} Acc:{:.3f}%, Number:{:.3f}% \n".format(j+1, differ_now, topn_accuracy_bydiffer[j] / total_bydiffer * 100, total_bydiffer/data_size * 100))
                total_bydiffer = 0
                topn_accuracy_bydiffer = [0 for _ in range(opt.n_best)]
                differ_now = item[0]
        ...
        for i in range(opt.n_best):
            if (i+1) in print_topk:
                if total_chirality > 0:
                    ...
                    OUT_FP.write("Top-{} Accuracy with chirality:{:.3f}% \n".format(i + 1, topn_accuracy_chirality[i] / total_chirality * 100))
                ...
                OUT_FP.write("Top-{} Accuracy without chirality:{:.3f}% \n".format(i + 1, topn_accuracy_wochirality[i] / (data_size - total_chirality) * 100))
                if total_ringopening > 0:
                    ...
                    OUT_FP.write("Top-{} Accuracy ring Opening:{:.3f}% \n".format(i + 1, topn_accuracy_ringopening[i] / total_ringopening * 100))
                if total_ringformation > 0:
                    ...
                    OUT_FP.write("Top-{} Accuracy ring Formation:{:.3f}% \n".format(i + 1, topn_accuracy_ringformation[i] / total_ringformation * 100))
                ...
                OUT_FP.write("Top-{} Accuracy without ring:{:.3f}% \n".format(i + 1, topn_accuracy_woring[i] / (data_size - total_ringopening - total_ringformation) * 100))
        print(total_chirality)
        print(total_ringformation)
        print(total_ringopening)
        OUT_FP.write('{} {} {}'.format(total_chirality, total_ringformation, total_ringopening))
